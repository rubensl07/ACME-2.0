...
import { getAtoresSort, validarData, getUsuario, deleteAtor} from "../../funcoes.js";
...
const containers = document.getElementById('atores')


iniciarTela()
async function iniciarTela(){
    ordem++
    listaAtores = (await getAtoresSort(alterarOrdem()))
...
document.getElementById('botaoDeletar').addEventListener('click',()=>{
        const divs = document.querySelectorAll('.ator');
        const lista = []; 
...
function criarContainer(info){
    ...
    ator.classList.add('ator', 'flex', 'bg-neutral-100', 'ml-8', 'rounded-lg', 'w-5/6', 'h-20'); 
...
barraCinza.forEach(barra => {
    ...
    ator.replaceChildren(nullSpace,barraCinza[0],id,barraCinza[1],nome,barraCinza[2],nascimento,barraCinza[3],sexo)
    containers.appendChild(ator)

    nome.addEventListener('click', ()=>{   window.location.href='./telaModificar.html?idAtor='+info.id
})
}
